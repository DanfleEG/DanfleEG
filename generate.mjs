...
// The GraphQL API always returns the LIGHT palette in `day.color`, which reads as
// white-on-black when painted over a dark canvas. Map `contributionLevel` to each
// theme's own palette instead, and emit one SVG per theme.
const THEMES = {
  dark: {
    file: "dist/github-jet.svg",
    canvas: "#0d1117",
    star: "#8b949e",
    levels: {
  ...
  light: {
    file: "dist/github-jet-light.svg",
    canvas: "#ffffff",
    star: "#8b949e",
    levels: {
      NONE: "#ebedf0", FIRST_QUARTILE: "#9be9a8", SECOND_QUARTILE: "#40c463",
      THIRD_QUARTILE: "#30a14e", FOURTH_QUARTILE: "#216e39",
    },
  },
};
  ...
  padded.forEach((week, col) => {
    week.contributionDays.forEach((day, row) => {
      cells.push({
        ...
        x: GRID_X + col * STEP,
        y: GRID_Y + row * STEP,
        color: theme.levels[day.contributionLevel] || theme.levels.NONE,
...
async function main() {
  ...
  for (const [name, theme] of Object.entries(THEMES)) {
    const outPath = path.resolve(theme.file);
    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, buildSvg(weeks, theme), "utf8");
